###############################################################################
# Grazer abundance (Main analyses)

#   Plate-level summer means (AvgGrazers)
#   Final Gaussian LMM on log(AvgGrazers + 1) 
#   * Random effects: (1 + Pollution | Site) only.
...
standardize_ci <- function(df) {
  ...
  df %>% dplyr::rename(lower = dplyr::all_of(lower_name),
                       upper = dplyr::all_of(upper_name))
}
...
#
# Grazers are recorded in components (surface/side); we consolidate to a single count per pad visit.
data_prep <- data_raw %>%
  mutate(
    Warming   = if_else(`0-Pad colour` == "B", 1L, 0L),
    Pollution = if_else(`0-Pad region` == "P", 1L, 0L),
    `3-Grazer total count` =
      `3-Grazer total count on pad surface` + `3-Grazer total count on pad sides`
  ) %>%
  dplyr::select(
...
pad_summary <- data_prep %>%
  ...
  filter(is.finite(AvgGrazers))

## 1.4 Drop 'zero-grazer' sites (mean == 0 across summer)
# These sites contribute no information about proportional change in grazer abundance.
zero_sites <- pad_summary %>%
  group_by(`0-Country/site`) %>%
  summarise(mu = mean(AvgGrazers, na.rm = TRUE), .groups = "drop") %>%
  filter(mu == 0) %>%
  pull(`0-Country/site`)
print(zero_sites) # All sites contain grazers

## 1.5 Build model response + plot keys
pad_summary <- pad_summary %>%
  filter(!(`0-Country/site` %in% zero_sites)) %>%
  mutate(
...
## 2.1 Final Gaussian LMM on log(AvgGrazers + 1)
# Random effects:
#   (1 + Pollution | Site) only.
#
# Rationale (grazer-specific):
#   Warming and Warming×Pollution random slopes were singular/unstable.
#   We retain the strongest supported site heterogeneity (Pollution slope),
#   and we compute site-level Warming/Interaction effects via empirical contrasts.
#   REML=FALSE (ML) to keep model comparison conventions stable across scripts.
graz_lmm <- lmerTest::lmer(
  log_AvgGrazers ~ Warming * Pollution +
    (1 + Pollution | `0-Country/site`),
  data = pad_summary,
  REML = FALSE
)
summary(graz_lmm)
...
conv_msgs <- tryCatch(graz_lmm@optinfo$conv$lme4$messages, error = function(e) NULL)

model_check_tbl <- tibble(
  singular      = lme4::isSingular(graz_lmm, tol = 1e-4),
...
fe_tbl <- broom.mixed::tidy(graz_lmm, effects = "fixed", conf.int = TRUE) %>%
  rename(df_obs = df) %>%
  transmute(
    term,
...
## 3.2 EMM contrasts
emm_link <- emmeans(graz_lmm, ~ Warming * Pollution, type = "link")
...
extract_interaction_class_log <- function(model,
                                          alpha    = 0.05,
                                          equiv_lo = EQUIV_LO,
                                          equiv_hi = EQUIV_HI,
                                          label    = "Whole Season") {
  ...
  if (any(is.na(c(b0, bw, bp, bwp, pwp, lo, hi)))) {
    stop("extract_interaction_class_log(): Missing fixed-effect term(s) or CI columns.")
  }
  ...
  overlaps_equiv <- !(ratio_hi < equiv_lo || ratio_lo > equiv_hi)
  ...
  tibble(
    ...
    Ratio_CI          = sprintf("[%.3f, %.3f]", ratio_lo, ratio_hi),
    p_value           = pwp,
    Class             = class,
    Additive_log      = d_add,
    Observed_log      = d_obs,
...
int_class_tbl <- extract_interaction_class_log(graz_lmm, label = "Whole Season")
print(int_class_tbl)
...
emm_link_cells <- emmeans(graz_lmm, ~ Warming * Pollution, type = "link")

pred_df <- summary(emm_link_cells, infer = TRUE) %>%
  as.data.frame() %>%
...
site_means <- raw_df %>%
  ...
  summarise(site_avg = mean(AvgGrazers, na.rm = TRUE), .groups = "drop") %>%
...
p_np <- ggplot() +
  ...
  geom_jitter(
    data = raw_np,
    aes(x = Warming_x, y = AvgGrazers + PSEUDO_COUNT, fill = TreatFill),
  ...
  scale_y_continuous(
    name   = expression(Grazer~count~"(log"[10]*" scale)"),
    trans  = "log10",
    breaks = c(1, 3, 10, 30),
...
# Pollution effect:
#   Supported random slope -> BLUP deviation only
#
# Warming + Interaction:
#   Not supported as random slopes -> use empirical within-site cell-mean contrasts:
#     Warming (NP only) = (W1P0 - W0P0)
#     Interaction       = (W1P1 - W1P0) - (W0P1 - W0P0)
...
# Random effects (BLUPs) and posterior var arrays for supported RE terms
re_site  <- ranef(graz_lmm, condVar = TRUE)$`0-Country/site`
pv_array <- attr(re_site, "postVar")  # [nTerms x nTerms x nSites]
re_terms <- colnames(re_site)

if (!("Pollution" %in% re_terms)) {
  stop("Expected random-slope term `Pollution` not found in ranef().")
}

# Fixed effects and their vcov matrix
fix_eff <- fixef(graz_lmm)
vc_fix  <- as.matrix(vcov(graz_lmm))

# 4.0 Pollution BLUP effect (log scale)
poll_df <- tibble(
  Site_raw  = rownames(re_site),
  Site      = clean_site(rownames(re_site)),
  Term      = "Pollution",
  slope_log = re_site[, "Pollution"],
  se_log    = safe_sqrt(
    pv_array[which(re_terms == "Pollution"), which(re_terms == "Pollution"), ]
  )
) %>%
  mutate(
...
# 4.0 Empirical cell means (log scale) for Warming + Interaction contrasts
cell_stats <- pad_summary %>%
  ...
  mutate(
    sd_log = if_else(is.na(sd_log), 0, sd_log),
    se_log = if_else(n > 0, sd_log / sqrt(n), NA_real_)
  )
...
emp_df <- purrr::map_dfr(sites_all, function(s) {
  ...
  # If any cell is missing, contrasts cannot be computed.
  ...
  tibble(
    Site_raw  = s,
    Term      = c("Warming", "Interaction"),
    slope_log = c(w_np, int_dd),
    se_log    = c(se_w_np, se_int_dd)
  )
}) %>%
  mutate(
...
effects_long <- bind_rows(
  emp_df  %>% select(Site, Term, slope_log, se_log, lower_log, upper_log, pct, lower_pct, upper_pct),
  poll_df %>% select(Site, Term, slope_log, se_log, lower_log, upper_log, pct, lower_pct, upper_pct)
) %>%
  mutate(Term = factor(Term, levels = c("Warming", "Pollution", "Interaction")))
...
ord_sites <- site_info %>%
  semi_join(effects_long %>% distinct(Site), by = "Site") %>%
...
p_log_graz <- ggplot() +
  ...
  geom_errorbarh(
    data   = effects_long %>% mutate(Site = as.character(Site)),
  ...
  scale_x_continuous(name = "Log change in mean grazer count (log(Avg+1) scale)") +
  scale_y_discrete(
    limits = y_levels,
...
make_biogeog_tables <- function(effects_df, pad_summary, response_label, pct_col) {
  ...
  zone <- ef %>%
    ...
    mutate(
      across(c(Mean_Temperate, Mean_NonTemp, Diff_TminusN, t), ~round(., 2)),
      p = signif(p, 3)
    )
...
graz_biog <- make_biogeog_tables(
  effects_df      = effects_long,
  pad_summary     = pad_summary,
  response_label  = "Grazers",
  pct_col         = "pct"
)

print(graz_biog$zone)
print(graz_biog$latitude)

## 5. Meta-regression: site-level warming sensitivity --------------------------

site_warm_base <- cell_stats %>%
  transmute(
    Site_raw  = `0-Country/site`,
    Site      = clean_site(`0-Country/site`),
    Pollution = if_else(Pollution == 0, "Non-Polluted", "Polluted"),
    Warming   = if_else(Warming == 0, "Ambient", "Warmed"),
    mean_log,
    se_log
  ) %>%
  tidyr::pivot_wider(
  ...
  mutate(
    slope_log = mean_log_Warmed - mean_log_Ambient,
    se_slope  = sqrt(se_log_Warmed^2 + se_log_Ambient^2),
...
pad_max_temps <- tibble(
  Site_raw = rep(
    c("Argentina","Chile","Ecuador","NZ","Denmark",
      "Greenland","Norway","Portugal1","Portugal2","UK","Vietnam1"),
    each = 2
  ),
  group   = rep(c("Ambient", "Warmed"), times = 11),
  MeanMax = c(
    29.55833, 31.08333,
    35.40000, 36.82727,
    41.26667, 41.73333,
...
Grazers_Warm_TableA <- dplyr::bind_rows(
  {
    s <- df_dw %>% filter(Pollution == "Non-Polluted")
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Stratum = "Non-polluted"),
  ...
  },
  {
    ...
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Stratum = "Polluted"),
      tidy_cor(s$DeltaWarming, s$warm_pct),
      tidy_slope(s$DeltaWarming, s$warm_pct)
    )
  }
...
Grazers_Warm_TableB <- dplyr::bind_rows(
  {
    s <- df_lat
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Moderator = "Absolute latitude (°)"),
  ...
  },
  {
    s <- df_max
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Moderator = "Mean daily max (°C)"),
  ...
  },
  {
    s <- df_sd
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Moderator = "SD of daily temp range (°C)"),
...
Grazers_Warm_TableC <- dplyr::bind_cols(
  tibble::tibble(Response = "Grazers", Stratum = "Non-polluted"),
...
print(Grazers_Warm_TableA)
print(Grazers_Warm_TableB)
print(Grazers_Warm_TableC)
...
Grazers_Poll_TableA <- dplyr::bind_rows(
  {
    s <- df_meta_poll_lnR %>% filter(Warming == "Ambient")
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Stratum = "Ambient"),
  ...
  },
  {
    ...
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Stratum = "Warmed"),
      tidy_cor(s$lnR_NO3, s$poll_pct),
      tidy_slope(s$lnR_NO3, s$poll_pct)
    )
  }
...
Grazers_Poll_TableB <- dplyr::bind_rows(
  {
    s <- df_amb
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Moderator = "Human land cover (%)"),
  ...
  },
  {
    s <- df_amb
    dplyr::bind_cols(
      tibble::tibble(Response = "Grazers", Moderator = "Mean precipitation (mm/day)"),
...
print(Grazers_Poll_TableA)
print(Grazers_Poll_TableB)
...
Grazers_Base_precip_mu <- dplyr::bind_cols(
...
Grazers_LOO_precip_mu <- loo_analyse(dat_prec, "precip_mu", "poll_pct") |>
...
if (length(vn_hit) > 0) {
  ...
  Grazers_DropVietnam <- dplyr::bind_rows(
    dplyr::bind_cols(
  ...
  Grazers_DropVietnam <- tibble::tibble(
...
lin_mu_g <- stats::lm(poll_pct ~ precip_mu, data = dat_prec)
Grazers_Influence_mu <- infl_tbl(lin_mu_g, dat_prec, "poll_pct ~ precip_mu")

Grazers_Base_precip_mu <- Grazers_Base_precip_mu |>
  dplyr::mutate(dplyr::across(where(is.numeric), ~ round(., 3)))

Grazers_LOO_precip_mu <- Grazers_LOO_precip_mu |>
  dplyr::mutate(dplyr::across(c(r, r_L, r_U, p, slope, s_L, s_U), ~ round(., 3)))

Grazers_Influence_mu <- Grazers_Influence_mu |>
  dplyr::mutate(dplyr::across(where(is.numeric), ~ round(., 3)))

print(Grazers_Base_precip_mu)
print(Grazers_LOO_precip_mu)
print(Grazers_DropVietnam)
print(Grazers_Influence_mu)